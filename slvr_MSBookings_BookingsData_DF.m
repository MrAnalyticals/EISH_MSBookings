...
- **Dimension Tables** = Contains descriptive attributes (services, staff)
  ...
  */
            ...
            AppointmentsTable = 
                ...
                then 
                    ...
                    let
                        ...
                        // Handle multiple possible formats: nested record, direct string, or direct datetime
                        ExtractStart = Table.AddColumn(
                            ExpandedTable, 
                            "StartDateTime", 
                            each 
                                if [Start] = null then null
                                else if Value.Is([Start], type record) then 
                                    try [Start][dateTime] 
                                    otherwise try [Start][DateTime] 
                                    otherwise null
                                else if Value.Is([Start], type text) then [Start]
                                else if Value.Is([Start], type datetime) then DateTime.ToText([Start], "yyyy-MM-ddTHH:mm:ss")
                                else null
                        ),
                        ...
                        ExtractEnd = Table.AddColumn(
                            ...
                            "EndDateTime", 
                            each 
                                if [End] = null then null
                                else if Value.Is([End], type record) then 
                                    try [End][dateTime] 
                                    otherwise try [End][DateTime] 
                                    otherwise null
                                else if Value.Is([End], type text) then [End]
                                else if Value.Is([End], type datetime) then DateTime.ToText([End], "yyyy-MM-ddTHH:mm:ss")
                                else null
                        ),
  ...
  // Handle multiple formats: ISO 8601, already datetime, or null
  ConvertDates = Table.TransformColumns(
    ExpandAppointments, 
    {
      {"StartDateTime", each 
        if _ = null then null
        else if Value.Is(_, type datetime) then _
        else if Value.Is(_, type text) then 
          try DateTime.FromText(_, [Format="yyyy-MM-ddTHH:mm:ss"]) 
          otherwise try DateTime.FromText(_) 
          otherwise null
        else null, 
  ...
  ReorderColumns = Table.ReorderColumns(AddYear, {"BusinessId", "BusinessName", "BusinessEmail", "BusinessPhone", "AppointmentId", "CustomerName", "CustomerEmail", "CustomerPhone", "ServiceId", "ServiceName", "StartDateTime", "EndDateTime", "AppointmentDate", "DayOfWeek", "Month", "Year", "Duration", "IsLocationOnline", "OnlineMeetingUrl", "AdditionalInfo"}),
  ...
  SetDataTypes = Table.TransformColumnTypes(ReorderColumns, {{"BusinessId", type text}, {"BusinessName", type text}, {"AppointmentId", type text}, {"CustomerName", type text}, {"CustomerEmail", type text}, {"ServiceName", type text}, {"Duration", type text}, {"IsLocationOnline", type logical}}),
