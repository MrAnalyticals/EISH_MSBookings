...
3. **Why This Matters:**
   ...
   - Accurate multi-staff tracking: Appointments with multiple staff expand to multiple rows

**Example DAX Measure (works because of relationships):**
```dax
Total Appointments = DISTINCTCOUNT(Bookings_Appointments[AppointmentId])
Total Appointment Rows = COUNTROWS(Bookings_Appointments)  // Higher due to multi-staff
